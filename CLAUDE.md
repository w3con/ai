...
**What every plan must contain.** This is the standard a plan meets before anyone else looks at it, not a list to repair afterwards. Writing a plan that lacks any of it, and then bolting on the missing checkpoint list, alternatives section, reuse line, dependency line or status column once someone points at the hole, costs a full rewrite to discover what was knowable while writing — and it has happened over and over. Put all of it in the first draft.

*At the level of the plan as a whole:* a one-paragraph statement of what is being built and why, readable cold by a future session; an explicit list of what must NOT be touched; acceptance criteria for the whole task, not only per phase; a `Status:` field in the header plus a per-phase status that can move from pending through in-progress to done or failed; and the work's registry identifier in the form `PREFIX-number`, where the project keeps a registry of work at all.

*In every single phase, without exception:* **one durable artifact** — one new file, or one coherent set of edits to one existing file; **acceptance criteria concrete enough for someone who did not write the plan to check** — named content, named behaviour, a named error that must not occur, never "it works"; **survivability** — if the session dies right after this phase, a new session opens the plan, sees the phase done, and resumes at the next one without losing work; **no more than three or four substantial file operations**, and a phase that needs more gets split; **the context to read first** — which files, which earlier phases must be closed, which outside facts are needed; **who executes it** — a fresh subagent or the coordinator inline; **alternatives with their cost** — the other ways this could have been done and roughly what each would cost, so the chosen way is argued rather than asserted; **reinvention ruled out** — the existing mechanism that is reused, or an explicit reason for not reusing it; and **tickable checkpoints in the plan text itself** — `- [ ] X.n — …` lines, one per independently completable step, so a cold session can tell from the plan alone where to resume. That last point has a second limb that is easy to miss: a phase producing one artifact still needs internal sub-checkpoints if making that single artifact costs the executor more than roughly 30k tokens, because the yardstick is never the size of the phase but how much work is lost between two saves.
