...
description: Scope and phase non-trivial work before executing — Opus writes the phased plan, the scope agent checks it and records a verdict, then build work can proceed
metadata:
  type: feedback
  scope: user-level
---

Two facets of one rule: **estimate the size of the work before you spawn anything, and never start a non-trivial build without a phased plan that has passed scope review.**
...
**Non-trivial work → Opus writes a plan, then scope checks it.** Before executing any non-trivial task — more than 2–3 files, several distinct steps, or real risk of session exhaustion — Opus writes a phased plan to `~/.claude/plans/<slug>.md` (one file per task, kebab-slug name, no shared `current.md`). Then the `scope` agent (`~/.claude/agents/scope.md`) reads the plan, evaluates its phasing quality, and writes a `## Scope verdict — PASS|FAIL` block into the plan file. On PASS, scope writes the sentinel `<!-- scope:pass -->` into that block; the plan-gate hook requires this sentinel before any build or implementation agent can be spawned. I then show the plan (with verdict) to the user and get explicit approval **before writing a single line of implementation**.

The `scope` agent does NOT write plans. It only checks them. Opus writes the plan; scope validates it.
...
**How to apply:**
- Opus writes the phased plan, scope checks it and records a PASS or FAIL verdict. The plan-gate hook blocks build-agent spawns until `<!-- scope:pass -->` is present in a plan.
...
**Paradigm connection:** a phased, resumable on-disk plan with a scope PASS verdict is the required artifact before the loop's build phase begins. Without it, there is nothing to gate against and no durable checkpoint. The triage logic (count ops; trivial → inline; non-trivial → phased plan → scope check → user approval → build) is how the loop stays proportionate and trustworthy. Relates to [[feedback_opus_plans_only]] and [[feedback_decide_before_building]].
