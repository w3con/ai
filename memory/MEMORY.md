...
- [Думать — значит писать в чат](feedback-think-in-chat.md) — ход мысли пишется текстом в ответе до действия; одна строка вывода вместо разложенного рассуждения не годится
...
- [Run the card means the whole pipeline](feedback-run-the-card-means-the-whole-pipeline.md) — Validité app only: «делай/запускай карточку» — это критик, карта, передача, исполнитель, трассировка, приёмка и push разом, без переспрашивания на каждом шаге
