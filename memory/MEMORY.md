...
- [Scope before executing: plan non-trivial work first](feedback_scope_agent.md) — count the ops; mechanical/trivial → do inline; non-trivial → the `scope` agent writes a phased resumable on-disk plan for approval before any implementation; >3–4 file ops = split; the phased plan is the required on-disk artifact the loop's plan-arm must produce
