...
metadata: 
  ...
  modified: 2026-08-15T21:25:58.823Z
---
...
Parallel work goes into a **separate session**, and a second session on a project is started by cutting it its own copy of the repository — in the Validité DPP project that command is `bin/session-start`, and `/new` (`.claude/skills/new/`) is the shortcut that runs it. Never open a second window on the shared working copy: two sessions sharing one working tree and one staging area interleaved three commits in two minutes on 2026-08-07.
