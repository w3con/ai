...
**Paradigm connection:** this fact IS the loop in operational form. "Discuss → decide → implement" maps directly to LOOP → "Execute the plan" → execution. The MORE=LESS principle is the same insight from a different angle: the plan and the shared understanding are the deliverable; acting before "Execute the plan" is not a shortcut, it is a skip past the work. Every rule in this file is a concrete failure mode of the broader "execution is not your default function" constraint.
